...
function M.waitFor(conditionFunc, timeoutMs, checkIntervalMs, abortFunc)
...
function M.output(msg, ...)
    printf("\a-t[Squire]\aw " .. msg .. "\ax", ...)
end

function M.debugOutput(msg, ...)
    if M.debugMode then
        local t = mq.gettime()
...
function M.saveSettings(settings)
    mq.pickle(M.getSettingsPath(), settings)
end

function M.loadSettings()
    local defaults = defaultSettings()
    local configData, err = loadfile(M.getSettingsPath())
    if err or not configData then
        M.saveSettings(defaults)
        return defaults
    end
    ...
    for setName, set in pairs(settings.sets) do
        if type(set) == "table" then
            for i, entry in ipairs(set) do
                local def = M.defaultSourceEntry()
...
function M.getNumPackSlots()
    return mq.TLO.Me.NumBagSlots()
end

function M.findFreeTopSlot()
    for i = 1, M.getNumPackSlots() do
...
function M.freeTopSlot()
    ...
    for i = 1, M.getNumPackSlots() do
        ...
        if packSlot.Item.ID() and (not packSlot.Item.Container() or packSlot.Item.Container() == 0) then
            sourceSlot = i
            M.debugOutput("freeTopSlot: found non-container item in pack%d: %s", i, packSlot.Item.Name() or "?")
            break
        end
    end
    ...
    if not sourceSlot then
        M.debugOutput("freeTopSlot: no non-container items, looking for empty containers")
        for i = 1, M.getNumPackSlots() do
            local packSlot = mq.TLO.InvSlot("pack" .. i)
            ...
            if container and container > 0 then
                ...
                if isEmpty then
                    sourceSlot = i
                    M.debugOutput("freeTopSlot: found empty container in pack%d: %s (%d slots)", i, packSlot.Item.Name() or "?", container)
                    break
                end
            end
        end
    ...
    if not sourceSlot then
        M.output("\arNo movable top-level item or empty bag found. Cannot free a slot.")
        return "abort"
    end
    ...
    if not destPack then
        M.output("\arNo bag has a free sub-slot. Cannot free a top-level slot.")
        return "abort"
    end

    M.debugOutput("freeTopSlot: moving pack%d → pack%d slot %d", sourceSlot, destPack, destSubSlot)
    ...
    if not mq.TLO.Cursor.ID() then
        M.output("\arFailed to pick up item from pack%d.", sourceSlot)
        return "abort"
    end
    ...
    if mq.TLO.Cursor.ID() then
        M.output("\arFailed to place item into pack%d slot %d. Cursor stuck.", destPack, destSubSlot)
        return "abort"
    end

    M.debugOutput("freeTopSlot: freed pack%d", sourceSlot)
    return sourceSlot
end

function M.ensureFreeTopSlot()
    local freeSlot = M.findFreeTopSlot()
    if freeSlot then
        M.debugOutput("ensureFreeTopSlot: pack%d already free", freeSlot)
        return freeSlot
    end
    M.debugOutput("ensureFreeTopSlot: no free top slot, attempting to free one")
    return M.freeTopSlot()
end

function M.clearCursor(needTopSlot)
    if not mq.TLO.Cursor.ID() then
        return nil
    end
    M.debugOutput("clearCursor: %s (ID: %d), needTopSlot=%s", mq.TLO.Cursor.Name() or "?", mq.TLO.Cursor.ID(), tostring(needTopSlot))

    if not needTopSlot then
        mq.cmd("/autoinventory")
        mq.delay(3000, function() return not mq.TLO.Cursor.ID() end)
        if mq.TLO.Cursor.ID() then
            M.output("\arFailed to autoinventory cursor item.")
            return "abort"
        end
        return nil
    end
    ...
    local destPack, destSubSlot = M.findBagWithSpace()
    if not destPack then
        M.output("\arCannot clear cursor: no bag has a free sub-slot.")
        return "abort"
    end
    ...
    mq.delay(3000, function() return not mq.TLO.Cursor.ID() end)
    if mq.TLO.Cursor.ID() then
        M.output("\arFailed to place cursor item into pack%d slot %d.", destPack, destSubSlot)
        return "abort"
    end
